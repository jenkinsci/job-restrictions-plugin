...
public class UserIDCauseRestrictionPipelineTest {
    ...
    public void nodeShouldAllowPipelineRunsByAcceptedUser_UserIdCause() throws Exception {
        ...
        slave.getNodeProperties()
                .add(new JobRestrictionProperty(new StartedByUserRestriction(listUserSelector, true, true, false)));
    ...
    public void nodeShouldNotAllowPipelineRunsByUnacceptedUser_UserIdCause() throws Exception {
        ...
        List<StepExecution> executions =
                run.getExecutionPromise().get().getCurrentExecutions(true).get();
        ...
        assertThat(
                "There is no buildable items in the queue. The job is not blocked", buildableItems.size(), equalTo(1));
